...
def build_vision_client(settings: Settings | None = None) -> AIClient:
    """Client per gli allegati immagine (usa ai_vision_model)."""
    settings = settings or get_settings()
    provider = settings.ai_provider.lower()
    if provider == "ollama":
        return OllamaClient(
    ...
    raise AIError(f"ai_provider non supportato per vision: {settings.ai_provider!r}")
